...
private:
    ...
    {
    public:
        KnobWindow(KnobSlider* host) : _host(host) { }
        void draw2()
        {
            Vector s = size();
            Byte* row = data();
    ...
    public:
        ...
        void update(Vector position)
        {
            ...
            fillParallelogram(_bitmap.subBitmap(Vector(0, 0), s), &corners[0],
                _host->_lightGrey);
    ...
    private:
        static bool insideParallelogram(Vector2<double> p,
        ...
        static bool lineSegmentsIntersect(Vector2<double> a0,
            Vector2<double> a1, Vector2<double> b0, Vector2<double> b1)
        {
        ...
        {
            Vector s = bitmap.size();
            if (s.x > s.y) {
                int sx2 = s.x/2;
                Vector2<double> s2(sx2, s.y);
                fillParallelogram(bitmap, points, colour,
                    Vector2<double>(0, 0), s2);
                fillParallelogram(bitmap, points, colour,
                    Vector2<double>(sx2, 0), s2);
            }
            else {
                int sy2 = s.y/2;
            ...
            Vector2<double> size)
        {
            ...
            if (allOutside) {
                ...
                for (int i = 0; i < 4; ++i) {
                    for (int j = 0; j < 4; ++j) {
                        if (lineSegmentsIntersect(points[i],
                            points[(i + 1) & 3], rect[j], rect[(j + 1) & 3])) {
                            intersects = true;
                            i = 4;
                            j = 4;
                        }
            ...
            int area =
                fillParallelogram(_bitmap, points, colour, tl, size - s) +
                fillParallelogram(_bitmap, points, colour, tl + s, size - s);
            if (size.x == 1 && size.y == 1) {
                ...
                double gamma = 2.2;
                double rLinear = pow(GetRValue(colour)/255.0, gamma);
                double gLinear = pow(GetGValue(colour)/255.0, gamma);
                double bLinear = pow(GetBValue(colour)/255.0, gamma);
                ...
                int rSRGB = static_cast<int>(
                    pow(bRLinear, 1/gamma)*255.0*coverage + 0.5);
                int gSRGB = static_cast<int>(
                    pow(bGLinear, 1/gamma)*255.0*coverage + 0.5);
                int bSRGB = static_cast<int>(
                    pow(bBLinear, 1/gamma)*255.0*coverage + 0.5);
    ...
    // Note: the values returned from GetSysColor have the red channel in the
