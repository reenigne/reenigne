...
//            }
//
            bool write = _ioType == IOType::writeMemory ||
                _ioType == IOType::writePort;
            _bus.wait();
            bool ready = true;
            switch (_busState) {
                case BusState::t1:
...
//                // stub gets desynchronized, set logStartCycle = 0 here and first
...
//    }
    Word busRead(IOType type = IOType::readMemory)
    {
        if (_wordSize)
            _m = busReadWord(type);
        else
            _m = busReadByte(type) | 0xff00;
        return _m;
    }
...
//            return;
        if (!_wordSize)
            _r = readByteRegister(modRMRReg());
        else
            _r = _wordRegisters[modRMRReg()];

        if ((_modRM & 0xc0) != 0xc0) {
            if (!_wordSize)
                _m = readByteRegister(modRMMReg());
            else
                _m = _wordRegisters[modRMMReg()];
            return;
        }
        wait(1);
        if ((_modRM & 0xc7) == 0x06) {
...
//    Word& rw() { return _wordRegisters[_opcode & 7]; }
    ...
    int modRMRReg() { return (_modRM >> 3) & 7; }
    int modRMMReg() { return _modRM & 7; }
...
//    int _timeSeg1;
    ...
    Word _r;
    Word _m;
    Word _address; // called IND in microcode
    Word _opr;
//    Word _tmpa;
    Word _tmpb;
//    Word _tmpc;
