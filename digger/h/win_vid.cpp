...
{
  windows_finish();
  MessageBox(hWnd, szError, L"Digger", MB_OK);
  DestroyWindow(hWnd);
  exit(1);
  return hRet;
}
...
HRESULT init_surfaces()
{
  ...
  if (g_bWindowed)
  {
    ...
    hDC=GetDC(hWnd);
    palettized_desktop = ((GetDeviceCaps(hDC, RASTERCAPS)&RC_PALETTE) != 0);
    if (palettized_desktop)
    {
      logpalette=(LOGPALETTE*) malloc(sizeof(LOGPALETTE) + sizeof(PALETTEENTRY)*64);
      if (!logpalette)
        fatal_error(0, L"init_surfaces: could not allocate memory for 'logpalette'");
