...
WCHAR backup_levfname[FILENAME_BUFFER_SIZE];
bool backup_levfflag;

WCHAR drf_filename[FILENAME_BUFFER_SIZE]=L"";
  ...
  case WM_COMMAND:
    ...
    case ID_RECORDING_PLAY:
      if (get_open_save_filename(OPEN, "Open Recorded Game","Digger Record Files (*.drf)\0*.DRF\0All Files (*.*)\0*.*\0","DRF", drf_filename))
      {
        ...
        wcscpy(backup_levfname,levfname);
        ...
        wcscpy(levfname,backup_levfname);
...
bool create_window(HINSTANCE hInstance, int nShowCmd)
{
  ...
  wndClass.lpszClassName = L"Digger";
...
void windows_finish()
{
  ...
  if (diggers>1)
    WriteINIString(INI_GAME_SETTINGS, "Players", L"2S", ININAME);
  else
...
#ifdef WIN32
  use_performance_counter = (QueryPerformanceFrequency((LARGE_INTEGER*) &performance_frequency) != 0);
  if (!use_performance_counter)
    MessageBox(hWnd, L"QueryPerformanceCounter not supported by this computer.\nDigger will use the less accurate multimedia timer instead.\nPlease report this warning.", L"Warning", MB_OK);
#endif
...
void show_mouse_cursor()
{
  ...
  SetCursor(LoadCursor((HINSTANCE) NULL, (LPWSTR) IDC_ARROW));
...
LRESULT CALLBACK sound_settings_dialog_proc(HWND hDlg, UINT uMsg, WPARAM wParam, LPARAM lParam)
{
  ...
  case WM_COMMAND:
    ...
    case IDOK:
      ...
      soundflag = (IsDlgButtonChecked(hDlg, IDC_CHECK_PLAY_SOUNDS) != 0);
      musicflag = (IsDlgButtonChecked(hDlg, IDC_CHECK_PLAY_MUSIC) != 0);
...
LRESULT CALLBACK help_about_dialog_proc(HWND hDlg, UINT uMsg, WPARAM wParam, LPARAM lParam)
{
  WCHAR info_string[200];
  switch (uMsg)
  {
  case WM_INITDIALOG:
    cur_dialog_box=hDlg;
    MultiByteToWideChar(CP_UTF8, 0, DIGGER_VERSION, -1, info_string, 200);
    wcscat(info_string, L"\n");
#ifdef RUNTIMEDYNAMICLINK
    wcscat(info_string, L"Win32 version (DirectX optional)");
#else
 #ifdef WIN32
  #ifdef DIRECTX
    wcscat(info_string, L"Win32 DirectX version");
  #else
    wcscat(info_string, L"Win32 standard version");
  #endif
 #else
    wcscat(info_string, L"Win16 WinG version");
 #endif
#endif
    SetDlgItemText(hDlg, IDC_STATIC_VERSION, info_string);
    if (g_bWindowed)
    {
      wcscpy(info_string, L"Running in Windowed Mode\n");
      wcscat(info_string,palettized_desktop ? L"(256 Colors/Using Palette)\n" : L"(HighColor/trueColor)\n");
    }
    else
    {
      wcscpy(info_string, L"Running in Full Screen Mode\n");
    }
#ifdef RUNTIMEDYNAMICLINK
    wcscat(info_string, hDirectDrawInstance ? L"DirectDraw detected\n" : L"DirectDraw NOT detected\n");
#endif
#ifdef DIRECTX
    if (use_direct_draw==0)
      wcscat(info_string, L"Not using DirectDraw");
    else if (use_direct_draw==6)
      wcscat(info_string, L"Using DirectDraw 6");
#endif
    SetDlgItemText(hDlg, IDC_GRAPHICS_INFO, info_string);
    wcscpy(info_string, wave_device_available ? L"Available\n" : L"Unavailable\n");
#ifdef RUNTIMEDYNAMICLINK
    wcscat(info_string, hDirectSoundInstance ? L"DirectSound detected" : L"DirectSound NOT detected");
#endif
...
LRESULT CALLBACK levels_dialog_proc(HWND hDlg, UINT uMsg, WPARAM wParam, LPARAM lParam)
{
  ...
  int i;
  char str[4];
  WCHAR dlf_filename[FILENAME_BUFFER_SIZE];
...
bool get_open_save_filename(bool save, WCHAR* title, WCHAR* filter, char* defext, WCHAR* filename)
{
  OPENFILENAME ofn;
  WCHAR fn[FILENAME_BUFFER_SIZE];
  WCHAR dir[FILENAME_BUFFER_SIZE];
  bool result;
...
/* load a DLF file */
void load_level_file(WCHAR* fn)
{
  FILE* levf;
  levf=_wfopen(fn, L"rb");
  if (levf==NULL) {
    wcscat(fn, L".DLF");
    levf=_wfopen(fn, L"rb");
  }
