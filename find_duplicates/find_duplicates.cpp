...
class FindDuplicates : Uncopyable
{
    struct ObjectData
    {
    ...
    {
        DirectoryProcessor(FindDuplicates* map) : _map(map), _size(0) { }
...
public:
    FindDuplicates() : _duplicates(0) { }
    ObjectData operator()(const File& file)
    {
    ...
    ObjectData operator()(const Directory& directory)
    {
...
public:
    void run()
    {
        ...
        if (_arguments.count() == 1) {
            console.write("Usage: find_duplicates <path>\n");
            return;
        }
        for (int i = 1; i < _arguments.count(); ++i)
            applyToWildcard(map, _arguments[i], false);
        //map.dump();
        map.printDuplicates();
    }
};
